...
class WeightedSubsetRandomSampler(Sampler):
    ...
    def __init__(self, subset_indices, fullset_labels, rate=1):
        self.fullset_labels = fullset_labels
        self.subset_indices = subset_indices
        self.num_samples = len(self.subset_indices)
        self.rate = rate

        self.weights = torch.full((7,), 1. / self.num_samples, dtype=torch.float)
        print('INFO: resampling initiated, starting with uniform distribution: ', self.weights)

        self.samples_weight = None
        self.update_sample_weights()

    def update_sample_weights(self):
        samples_weight_fullset = np.array([self.weights[t] for t in self.fullset_labels])
    ...
    def fnc_expo(self, x, step):
        return x * (self.rate**step)

    def update_weights(self, epoch):
        new_val = self.fnc_expo(self.weights[1], epoch)
        for i in range(1,7,2):
            self.weights[i] = new_val

        self.update_sample_weights()
        print('INFO: updated weights, new weights: ', self.weights)
